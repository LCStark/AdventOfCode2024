...
fn main() -> io::Result<()> {    
    ...
    let incorrect_updates:Vec<&String> = updates.iter().filter(|x| !check_update(x, &before_rules)).collect();
    
    let fixed_updates: Vec<String> = incorrect_updates.iter().map(|x| fix_update(&x, &before_rules)).collect();
    
    let middle_values: Vec<u32> = fixed_updates.iter().map(|x| middle_value(&x).parse::<u32>().unwrap()).collect();
...
fn fix_update(update: &String, before_rules: &Vec<(String, Vec<String>)>) -> String {
    let mut split = update.split(",").collect::<Vec<&str>>();
    
    let mut current_index = 1;
    
    'outer: loop {
        let page = split[current_index].to_string();
        
        let ruleset_index = before_rules.iter().position(|x| x.0 == page);
        if ruleset_index.is_none() {
            current_index = current_index + 1;
            if current_index > split.len() - 1 {
                break;
            }
            continue;
        }
        ...
        for rule in &rule_set.1 {
            ...
            if rule_index.is_none() || rule_index.unwrap() > current_index {
                continue;
            }
            
            let rule_index = rule_index.unwrap();
            split.swap(current_index, rule_index);
            current_index = rule_index;
            continue 'outer;
        }
